...
export class Camera extends EventEmitter {
    ...
    async capture() {
        try {
            ...
            console.log('[Camera] Screenshot saved:', this.fp + filename + '.jpg');
            return filename;
        } catch (e) {
            console.error('[Camera] Capture failed:', e.message || e);
            return null;
        }
    }
    ...
    async _ensureScreenshotDirectory() {
        try {
            await fs.stat(this.fp);
        } catch (e) {
            console.log('[Camera] Screenshot directory does not exist, creating:', this.fp);
            await fs.mkdir(this.fp, { recursive: true });
            console.log('[Camera] Directory created:', this.fp);
        }
    }
}
